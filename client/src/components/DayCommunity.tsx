...
export function DayCommunity({ day }: DayCommunityProps) {
  const [activeTab, setActiveTab] = useState<"qa" | "discussion">("qa");
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h2 className="font-bold text-xl text-slate-900">Community</h2>
      </div>

      <Card className="border-2 border-slate-100 shadow-none bg-white overflow-hidden">
        {/* Tabs */}
        ...
        <div className="p-6">
          {activeTab === "qa" ? (
            ...
            <DiscussionTab day={day} isAdmin={isAdmin} />
          )}
        </div>
      </Card>
    </div>
      ...
      {!showForm ? (
        ...
        <div className="space-y-3 p-4 bg-slate-50 rounded-lg border border-slate-200">
          ...
          <p className="text-xs text-slate-500">
            Matt will be notified and may answer your question. Answers will appear here for everyone.
          </p>
        </div>
      )}
              ...
              {q.answer && (
                <div className="ml-11 p-3 bg-white rounded border border-slate-200">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-sm text-green-600 font-medium">Matt's Answer:</p>
                  </div>
...
function DiscussionTab({ day, isAdmin }: { day: number; isAdmin: boolean }) {
  const queryClient = useQueryClient();
