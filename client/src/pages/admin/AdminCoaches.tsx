    ...
    <div className="space-y-6">
                                ...
                                <div key={clientKey} className="bg-white rounded-lg border border-slate-200 overflow-hidden">
                                        ...
                                        <div className="flex items-center gap-3 mt-1 flex-wrap">
                                          ...
                                          {client.amountPaid > 0 && (
                                            <span className="text-xs text-green-600">
                                              Paid {formatMoney(client.amountPaid, client.currency)}
                                            </span>
                                          )}
                                          {cCompleted > 0 && (
                                            <span className="text-xs text-amber-600">
                                              Coach cost: {formatMoney(cCompleted * coach.ratePerSession, coach.rateCurrency)}
                                            </span>
                                          )}
                                        </div>
                                        {nextSession && (
                                  ...
                                  {isClientExpanded && (
                                    <div className="border-t border-slate-100 px-3 py-2 bg-slate-50 space-y-1.5">
                                      ...
                                      <div className="pt-2 flex items-center justify-between text-xs border-t border-slate-200 flex-wrap gap-2">
                                        <div className="text-slate-400 space-x-2">
                                          <span>Purchased {new Date(client.purchasedAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                                          {client.amountPaid > 0 && (
                                            <span className="text-green-600">Revenue: {formatMoney(client.amountPaid, client.currency)}</span>
                                          )}
                                          <span className="text-amber-600">
                                            Coach cost: {formatMoney(cCompleted * coach.ratePerSession, coach.rateCurrency)}/{formatMoney(client.sessionsTotal * coach.ratePerSession, coach.rateCurrency)}
                                          </span>
                                        </div>
                                        <select
                                          onChange={(e) => {
      ...
      <Card className="border border-slate-200 shadow-sm overflow-hidden">
        <button onClick={() => toggleSection('setup')} className="w-full p-5 flex items-center gap-3 text-left">
          ...
          <div className="flex-1">
            <h3 className="text-lg font-bold text-slate-900">Setup & Settings</h3>
            <p className="text-sm text-slate-500">
          ...
          {openSections.has('setup') ? <ChevronUp className="w-5 h-5 text-slate-400" /> : <ChevronDown className="w-5 h-5 text-slate-400" />}
        </button>
        {openSections.has('setup') && (
          <div className="px-5 pb-5 space-y-5">
            {/* Cal.com Integration */}
            {activeCoaches.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Cal.com Integration</p>
                <div className="bg-slate-50 rounded-lg p-4 border border-slate-200 space-y-3">
                  ...
                  <div className="space-y-2 pt-1">
                    <p className="text-sm font-medium text-slate-700">Send setup instructions:</p>
                    {activeCoaches.map((coach) => (
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-slate-200 flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-900">
                {previewType === 'email' ? 'Invitation Email Preview' : previewText.includes('Signed by:') ? 'Signed Contractor Agreement' : 'Contractor Agreement Preview'}
              </h3>
